...
# Edit by Tianyi Zhang on 30th June 2021: add l1 regularization and l1_l2 regularization
# main changes see line 232-247
...
class BaseMultilayerPerceptron(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self, hidden_layer_sizes, activation, solver,
                 alpha_l1, alpha_l2, batch_size, learning_rate, learning_rate_init, power_t,
    ...
    def _compute_loss_grad(self, layer, n_samples, activations, deltas,
        ...
        coef_grads[layer] = safe_sparse_dot(activations[layer].T,
                                            deltas[layer])
        coef_grads[layer] += (self.alpha_l2 * self.coefs_[layer])
    ...
    def _backprop(self, X, y, activations, deltas, coef_grads,
        ...
        # ======================= Edit by Tianyi Zhang ==================================
        l2_v = np.sum(np.array([np.dot(s.ravel(), s.ravel()) for s in self.coefs_]))
        l1_v = np.sum(np.abs(np.array(self.coefs_[0])))
        
        if self.alpha_l1 == 0.0000:
            # Add L2 regularization term to loss
            loss += (0.5 * self.alpha_l2) * l2_v / n_samples
        elif self.alpha_l2 == 0.0000:
            # Add L1 regularization term to loss
            loss += self.alpha_l1 * l1_v / n_samples
        else:
            # Add L1_L2 regularization term to loss
            r = self.alpha_l2 / (self.alpha_l2 + self.alpha_l1)
            l1_l2_v = (r * l2_v + (1 - r) * l1_v) / n_samples
            loss += l1_l2_v
        # ==================================================================================
        ...
        # print('intercept_grads', intercept_grads)
    ...
    def _validate_hyperparameters(self):
        ...
        if self.alpha_l1 < 0.0:
            raise ValueError("alpha_l1 must be >= 0, got %s." % self.alpha_l1)
        if self.alpha_l2 < 0.0:
            raise ValueError("alpha_l2 must be >= 0, got %s." % self.alpha_l2)
...
class MLPClassifier(ClassifierMixin, BaseMultilayerPerceptron):
    ...
    alpha_l1 : float, default=0
        L1 penalty (regularization term) parameter.

    alpha_l2 : float, default=0.0001
    ...
    def __init__(self, hidden_layer_sizes=(100,), activation="relu",
                 solver='adam', alpha_l1=0.000, alpha_l2=0.0001,
        ...
        super().__init__(
            hidden_layer_sizes=hidden_layer_sizes,
            activation=activation, solver=solver, alpha_l1=alpha_l1, alpha_l2=alpha_l2,
...
class MLPRegressor(RegressorMixin, BaseMultilayerPerceptron):
    ...
    def __init__(self, hidden_layer_sizes=(100,), activation="relu",
                 solver='adam', alpha_l1=0.0000, alpha_l2=0.0001,
                 batch_size='auto', learning_rate="constant",
